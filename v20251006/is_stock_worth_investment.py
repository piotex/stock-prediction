...
def is_stock_worth_interest(stock):
    ...
    # earnings_per_share = indicators.get('Zysk na akcję', [])  # brać z bankier - biznesradar źle liczy! -> profit_per_share
    ...
    eps_current = get_numeric_value(profit_per_share, -1)
    eps_previous = get_numeric_value(profit_per_share, -2) # Q-1
    ...
    eps_change_yr = calculate_change(profit_per_share, -1, change_type='r/r')
    ...
    stock["calculated_indicators"] = {
        "Stock Name": name,
        "Cena / Zysk": pe_ratio,
        "Cena / Wartość księgowa": pb_ratio,
        "Cena / Przychody ze sprzedaży": ps_ratio,
        "EV / EBITDA": ev_ebitda,
        "Wzrost EPS R/R": eps_change_yr,
        "Zmiana WK R/R": bvs_change_yr,
        "Zysk Operacyjny/Akcję": ebit_per_share_current,
    }
